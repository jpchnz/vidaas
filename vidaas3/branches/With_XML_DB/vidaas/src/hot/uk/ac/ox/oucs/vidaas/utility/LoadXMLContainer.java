...
public class LoadXMLContainer {

	public void loadXMLContainer() throws Throwable {
		File path2DbEnv = new File(
				"/opt/IDE/Netbeans-7.0/NetBeansProject/BDBXMLTest/examplesEnvironment");
		File filePath = new File(
				"/opt/IDE/Netbeans-7.0/NetBeansProject/BDBXMLTest/xmlTestFolder");
		...
		// This vector will hold a File object for each XML file that we will
		// load
		// into the examples container
		List<File> files2add = new LinkedList<File>();

		// Within .../examples/xmlData there should be two subdirectories,
		// nsData
		...
		File simpleData = new File(filePath.getPath() + File.separator
				+ "simpleData");
	...
	private static void confirmDirectory(File directory) {
		if (!directory.isDirectory()) {
			System.out.println("\nError. Directory " + directory.getPath()
					+ " does not exist.");
			System.out
					.println("       -p must point to the xmlData directory.");
			// usage();
		}
	}

	// Find all the xml files in a specified directory and store them in a
	// vector
	private static void getXmlFiles(File filePath, List<File> files2add) {
		...
		if (dirContents != null) {
			for (int i = 0; i < dirContents.length; i++) {
				File entry = new File(filePath + File.separator
						+ dirContents[i]);
				if (entry.isFile()
						&& entry.toString().toLowerCase().endsWith(".xml")) {
					files2add.add(entry);
					filesFound = true;
				}
			}
	...
	private static Environment createEnv(File home) throws DatabaseException,
	...
	private static void loadXmlFiles(File path2DbEnv, String theContainer,
		...
		try {
			...
			while (filesIterator.hasNext()) {
				...
				// Set the xml document's content to the xmlString we just
				// obtained.
				...
				xmlDoc.setMetaData(mdConst.uri, mdConst.name, new XmlValue(
						theDate.toString()));
			...
			txn.commit();
			// XmlException extends DatabaseException, which in turn extends
			// Exception.
			// Catching Exception catches them all.
		} catch (Exception e) {

			System.err.println("Error loading files into container "
					+ theContainer);
