...
$(function() {
    ...
    $("#oneToOne").click(function () { 
      if(oneToOneNonRelationSelected == true || oneToManyRelationSelected == true || oneToManyNonRelationSelected == true || manyToManyRelationSelected == true || manyToManyNonRelationSelected == true){
    ...
    $("#oneToOneNon").click(function () { 
      if(oneToOneRelationSelected == true || oneToManyRelationSelected == true || oneToManyNonRelationSelected == true || manyToManyRelationSelected == true || manyToManyNonRelationSelected == true){       
        resetRelationsButtons();  
      }  
    ...
    $("#oneToMany").click(function () {   
        if(oneToOneRelationSelected == true || oneToOneNonRelationSelected == true || oneToManyNonRelationSelected == true || manyToManyRelationSelected == true || manyToManyNonRelationSelected == true){
        resetRelationsButtons();  
        }       
    ...
    $("#oneToManyNon").click(function () {   
      if(oneToOneRelationSelected == true || oneToOneNonRelationSelected == true || oneToManyRelationSelected == true  || manyToManyRelationSelected == true || manyToManyNonRelationSelected == true){ 
        resetRelationsButtons(); 
        }       
    ...
    $("#manyToMany").click(function () {  
      if(oneToOneRelationSelected == true || oneToOneNonRelationSelected == true || oneToManyRelationSelected == true || oneToManyNonRelationSelected == true || manyToManyNonRelationSelected == true){    
        resetRelationsButtons();  
        }    
    ...
    $("#manyToManyNon").click(function () { 
      if(oneToOneRelationSelected == true || oneToOneNonRelationSelected == true || oneToManyRelationSelected == true || oneToManyNonRelationSelected == true || manyToManyRelationSelected == true){    
        resetRelationsButtons();      
      } 
    ...
    $("#columnType").change(function() { alert("select changed") });
          
	$( "#tableDrag" ).draggable({ distance: 20, addClasses: false, helper:'clone', opacity:0.7, 
	    start: function() {
			$( "body" ).find( "#testP" )
					.html( "Drag Started!" );
            ...
            $("#externalHtml").load('dialog/databaseForm.html').dialog({
                ...
                buttons: {
                    "Create Database": function() {
                        ...
                        var descriptionDescription = $('[name=databaseDescription]').val();
                        
                        databaseObject = createDatabaseFunction(databaseName,descriptionDescription); 
                        $( this ).dialog( "close" );  
                        
                        if(databaseObject != null){
                            $("#externalHtml").load('dialog/databaseConfirmation.html').dialog({
                                title: "Please, Create Database First",
                                modal: true,
                                autoOpen: true,
                                height: 140,
            ...
            $("#externalHtml").load('dialog/tableForm.html').dialog({
                ...
                buttons: {
                    "Create Table": function() {
                        ...
                        var tempInnerHTML = tempTable.createAndAddFunction(xPosition, yPosition, tableName, tableDescription, tempTable, databaseObject);
                        ...
                        // Add it table Twice in the array ... not sure why.....!
                        // Solution was to pass database object to creat and add function
                        //databaseObject.tables[tableCounter] = tempTable;
                        //alert(databaseObject.tables.length);	
	...
	$( "#columnDrag" ).draggable({ helper:'clone', opacity:0.7,
	    start: function() {
                ...
                $("#externalHtml").load('dialog/columnForm.html').dialog({
                   title: "Create Column",
                    ...
                    buttons: {
                            ...
                            var innerHTML = columnDragSelectedTable.updateFunction(columnKeyType, columnName, columnType, columnLength, columnScale, columnNull, columnUnique);
	...
	$('.tableElement').live('mousemove',function(event){ 
	/*
	    $(this).droppable({ accept:"#columnDrag",
	        drop: function(event, ui) {
			    alert("Dropped");
		    },
		    over: function(event, ui) {
			    //Core.getConsole().info("OVER");
			    alert("Over");
		    },
	    ...
	    $(this).draggable({containment: "#droppable",
	        handle: "th",
	        cursor: "move",
	        drag: function() {
	            ...
	            var tempTableOnMove = findTableInDatabase(tempID);
	            
	            tempTableOnMove.xPosition = $( "#" + tempID).offset().left;
                tempTableOnMove.yPosition = $( "#" + tempID).offset().top;
                
                var newXCoordinateForRelation = tempTableOnMove.xPosition + $("#" + tempID).width()/2;
                var newYCoordinateForRelation = tempTableOnMove.yPosition + $("#" + tempID).height()/2;
	            //alert(tempTableOnMove.relations.length);
	            //moveLine(event.pageX, event.pageY);
	            for(var i = 0; i < tempTableOnMove.relations.length; i++){
	                ...
	                updateRelation(tempRelationOnMove.line, tempRelationOnMove.orientation, newXCoordinateForRelation, tempTableOnMove.yPosition + 12/*event.pageX, event.pageY*/);
	            }
		    }
	    }); 
...
function tableToggleClick(element){
    ...
    if (oneToOneRelationSelected == false && oneToOneNonRelationSelected == false && oneToManyRelationSelected == false && oneToManyNonRelationSelected == false &&manyToManyRelationSelected == false && manyToManyNonRelationSelected == false){
        //alert(elementID);
      $(element).closest('div.tableElement').find('tbody')
        .toggle('slow',function(){
          $(element).closest('div.tableElement th')
...
function modifyColumn(columnID, tableID) {
    ...
    var tempID = "#" + tableID;
    var tempTable = findTableInDatabase(tableID);
    ...
    if(tempColumn != null) {
             ...
             title: "Update Column",
                ...
                buttons: {
                    "Update Column": function() {
                        ...
                        // Primary Here only for Test
                        var innerHTML = tempTable.updateColumnFunction(tempColumn, columnKeyType, columnName, columnType, columnLength, columnScale, columnNull, columnUnique);
      ...
      setTimeout(function(){
        ...
        } else {$('#columnOptional').attr('selectedIndex', 2);}
        
        if(tempColumnUnique == "UNIQUE"){
            $('#columnUnique').attr('selectedIndex', 1);
        } else {$('#columnUnique').attr('selectedIndex', 2);}
        ...
        } else {$("#columnPrimaryKey").attr('selectedIndex', 2);}
        parseDataTypeForColumDialog(tempColumnDataType, tempColumnLength);
        /*$('[name=tableDescription]').val(tempTable.description);*/}, 50);
  }
}
...
function modifyTable(tableID) {
   ...
   setTimeout(function(){$('[name=tableName]').val(tempTable.name); $('[name=tableDescription]').val(tempTable.description);}, 50);
...
function onMouseClickTable(element) {
    ...
    //alert($( tempID).height());
    //alert(element.width + " " + $( "#" +element.id).height() + "  " +  $( "#" +element.id).width() + " " + element.xPosition + "  " + $( "#" +element.id).offset().left);
    /*
    if (oneToOneRelationSelected == false && oneToOneNonRelationSelected == false && oneToManyRelationSelected == false && oneToManyNonRelationSelected == false &&manyToManyRelationSelected == false && manyToManyNonRelationSelected == false){
        //alert(elementID);
      $(element).closest('div.tableElement').find('tbody')
    ...
    if(oneToOneRelationSelected == true || oneToOneNonRelationSelected == true || oneToManyRelationSelected == true || oneToManyNonRelationSelected == true){
            if(counter == 0 ){
                //alert(event.pageX);
                lineX1 = $( "#" +element.id).offset().left;
                lineY1 = $( "#" +element.id).offset().top + 12;
                tempTable1 = findTableInDatabase(element.id);
                //alert("2: " + tempID);
                counter++;
            } else if (counter > 0 ){
                ...
                tempTable2 = findTableInDatabase(element.id);
                drawLine();
        ...
        } else if (manyToManyRelationSelected == true || manyToManyNonRelationSelected == true){
            ...
            if(counter == 0 ){
                ...
                tempTempTable2 = findTableInDatabase(element.id);
                //drawLine();
                ...
                var tempInnerHTML = tempTable.createAndAddWithoutPKFunction(lineX2, lineY2, tableName, tempTable, databaseObject);
                $(tempInnerHTML).appendTo($( "#droppable" ));                    
...
function drawLine(){
    ...
    tempLine = svg.line(g, lineX1 + table1Width/2, lineY1, lineX2 + table2Width/2, lineY2, {id:'relation_'+lineCounter});
    ...
    for (var i = 0; i < tempTable1.primaryKeys.length; i++){
        var innerHTML;
        if (oneToOneNonRelationSelected == true) {
                                                //keyTypeValue, keyNameValue,                                              dataTypeValue,                       keyUniqueValue, keyAutoIncrementValue
            innerHTML = tempTable2.updateFunction('Foreign',    tempTable1.name + "_"+ tempTable1.primaryKeys[i].keyName,  tempTable1.primaryKeys[i].dataType,  true,           false);            
        } else if (oneToOneRelationSelected == true || manyToManyRelationSelected == true || manyToManyNonRelationSelected == true) {
                                                //keyTypeValue, keyNameValue,                                              dataTypeValue,                       keyUniqueValue, keyAutoIncrementValue
            innerHTML = tempTable2.updateFunction('Primary',    tempTable1.name + "_"+ tempTable1.primaryKeys[i].keyName,  tempTable1.primaryKeys[i].dataType,  true,           false);
        } else if (oneToOneNonRelationSelected == true) {
                                                //keyTypeValue, keyNameValue,                                              dataTypeValue,                       keyUniqueValue, keyAutoIncrementValue
            innerHTML = tempTable2.updateFunction('Foreign',    tempTable1.name + "_"+ tempTable1.primaryKeys[i].keyName,  tempTable1.primaryKeys[i].dataType,  false,           false);
        } else if (oneToManyRelationSelected == true) {
                                                //keyTypeValue, keyNameValue,                                              dataTypeValue,                       keyUniqueValue, keyAutoIncrementValue
            innerHTML = tempTable2.updateFunction('Primary',    tempTable1.name + "_"+ tempTable1.primaryKeys[i].keyName,  tempTable1.primaryKeys[i].dataType,  false,           false);
        }
        
        newKeyInSecondryTable = findColumnInTableByName(tempTable2, tempTable1.name + "_"+ tempTable1.primaryKeys[i].keyName);
        ...
        //tempInnerHTML = tempInnerHTML + '<tr><td ondblclick="onMouseClickColumn(this)" id="' + this.primaryKeys[i].keyID +'">' + this.primaryKeys[i].keyName + '</td></tr>';
    }        
    lineCounter++;	    
}

function drawRelationFromXMLSchema(mainColumnNameValue, secondaryColumnNameValue){
    var g = svg.group({stroke: 'black', 'stroke-width': 2});
     var tempLine;
     var sx1, sy1, ex2, ey2;
    ...
    var keyInSecondryTable = findColumnInTableByName(tempTable2, secondaryColumnNameValue);
...
function svgOver() { 
    ...
    $(tempTable).closest('div.tableElement').find('thead > tr > th').css('background-color', '#FFCCCC').css('color','white');
    //$("#"+ tempRelation.mainTable.id).css('border', '40px');
    $("#"+ tempRelation.secondryTable.id).css('background-color', 'red');
    $("#"+ tempRelation.secondryTable.id).closest('div.tableElement').find('thead > tr > th').css('background-color', '#FFCCCC').css('color','white');
}
function svgOut() { 
    ...
    $("#"+ tempRelation.mainTable.id).closest('div.tableElement').find('thead > tr > th').css('background-color', '#D1D1D1').css('color','black');
    //$("#"+ tempRelation.mainTable.id).css('border', '1px');
    $("#"+ tempRelation.secondryTable.id).css('background-color', 'black');
    $("#"+ tempRelation.secondryTable.id).closest('div.tableElement').find('thead > tr > th').css('background-color', '#D1D1D1').css('color','black');
}
...
function updateRelation(line, orientationVal, xCoordinate, yCoordinate) {
    if(orientationVal == "start"){
        $(line).animate({svgX1: xCoordinate, svgY1: yCoordinate}, 0);
    } else if (orientationVal == "end") {
        $(line).animate({svgX2: xCoordinate, svgY2: yCoordinate}, 0);
    }	    
}
...
function moveLine(xTemp, yTemp) { 
    ...
    for (var i=0; i < lines$.length; i++) {
        $(lines$[i]).animate({svgX1: xTemp, svgY1: yTemp}, 0);
    }
    //$(tempLine$).remove();
}

function findTableInDatabase(tableID) {
    //alert(tableID);
...
function findTableInDatabaseByName(tableName) {
    ...
    for (var i = 0; i < databaseObject.tables.length; i++) {
        ...
        if(tempTableInDatabase.name == tableName){
...
function findTableForColumnDrag(xTemp, yTemp){
    ...
    for (var i = 0; i < databaseObject.tables.length; i++) {
        ...
        //alert(xTemp + " " + yTemp + "  " + startingX + " " + endingX + " " + startingY + " " + endingY);
        ...
        if(xTemp > startingX - 20 && xTemp < endingX + 20){
            if(yTemp > startingY - 20 && yTemp < endingY + 20){
                //alert (startingX + " " + endingX + " " + startingY + " " + endingY + " ");
...
function createRelationForLoadedXMLSchema(foreignKeyRelationArray){
    for (var i = 0; i < foreignKeyRelationArray.length; i++) {
        tempTable1 = findTableInDatabaseByName(foreignKeyRelationArray[i].mainTable);
        lineX1 = $( "#" +tempTable1.id).offset().left;
        lineY1 = $( "#" +tempTable1.id).offset().top + 12;
                
        tempTable2 = findTableInDatabaseByName(foreignKeyRelationArray[i].secondaryTable);
        lineX2 = $( "#" + tempTable2.id).offset().left;
        lineY2 = $( "#" + tempTable2.id).offset().top + 12;
        drawRelationFromXMLSchema(foreignKeyRelationArray[i].mainColumnName, foreignKeyRelationArray[i].secondaryColumnName);
